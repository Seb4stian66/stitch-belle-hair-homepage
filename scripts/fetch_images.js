...
(async () => {
    ...
    console.log('\n[2/2] 📸 Starte dynamischen Bild-Extraktor...\n');
    const kundenDatenPath = path.join(__dirname, '../kunden_daten.json');
    if (!fs.existsSync(kundenDatenPath)) {
        console.log("❌ Keine kunden_daten.json gefunden.");
        process.exit(1);
    }
    const kundenDaten = JSON.parse(fs.readFileSync(kundenDatenPath, 'utf8'));
    const targetUrl = kundenDaten.FACEBOOK_URL;
    
    if (!targetUrl || targetUrl.trim() === "" || !targetUrl.includes("facebook.com")) {
        console.log("⚠ Keine gültige FACEBOOK_URL in JSON gefunden. Beende Bild-Extraktor.");
        process.exit(0);
    }

    console.log('Navigating to facebook homepage: ' + targetUrl);
    ...
    let photosUrl = targetUrl;
    if (!photosUrl.endsWith('/')) photosUrl += '/';
    photosUrl += 'photos/';
    console.log('Navigating to facebook photos: ' + photosUrl);
    await page.goto(photosUrl, { waitUntil: 'networkidle2', timeout: 30000 });
